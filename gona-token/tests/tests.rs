//! Tests for the `gona_token` contract.
use gona_token::*;
use concordium_cis2::*;
...
fn test_init() {
    ...
    let events = init
        .events
        .iter()
        .map(|e| e.parse().expect("Parsing GonaEvent."))
        .collect::<Vec<GonaEvent>>();

    assert_eq!(events, [
        GonaEvent::Cis2Event(Cis2Event::Mint(MintEvent {
            token_id: TOKEN_ID_GONA,
            amount:   ContractTokenAmount::from(0u64),
            owner:    ALICE_ADDR,
        })),
        GonaEvent::Cis2Event(Cis2Event::TokenMetadata(TokenMetadataEvent {
            token_id:     TOKEN_ID_GONA,
...
fn test_set_metadata_url() {
    ...
    let update = chain
        .contract_update(SIGNER, BOB, BOB_ADDR, Energy::from(10000), UpdateContractPayload {
            amount:       Amount::zero(),
            receive_name: OwnedReceiveName::new_unchecked("gona_token.setMetadataUrl".to_string()),
    ...
    assert_eq!(events, [GonaEvent::Cis2Event(Cis2Event::TokenMetadata(TokenMetadataEvent {
        token_id:     TOKEN_ID_GONA,
        metadata_url: MetadataUrl {
...
fn test_account_transfer() {
    ...
    let events = deserialize_update_events(&update);
    assert_eq!(events, [GonaEvent::Cis2Event(Cis2Event::Transfer(TransferEvent {
        token_id: TOKEN_ID_GONA,
        amount:   TokenAmountU64(1),
        from:     ALICE_ADDR,
        to:       BOB_ADDR,
...
fn test_add_operator() {
    ...
    let update = chain
        .contract_update(SIGNER, ALICE, ALICE_ADDR, Energy::from(10000), UpdateContractPayload {
            amount:       Amount::zero(),
            receive_name: OwnedReceiveName::new_unchecked("gona_token.updateOperator".to_string()),
            address:      contract_address,
    ...
    let events = deserialize_update_events(&update);
    assert_eq!(events, [GonaEvent::Cis2Event(Cis2Event::UpdateOperator(UpdateOperatorEvent {
        operator: BOB_ADDR,
        owner:    ALICE_ADDR,
        update:   OperatorUpdate::Add,
    })),]);
    ...
    let invoke = chain
        .contract_invoke(ALICE, ALICE_ADDR, Energy::from(10000), UpdateContractPayload {
            amount:       Amount::zero(),
            receive_name: OwnedReceiveName::new_unchecked("gona_token.operatorOf".to_string()),
            address:      contract_address,
...
fn test_unauthorized_sender() {
    ...
    // Construct a transfer of `TOKEN_ID_GONA` from Alice to Bob, which will be
    // submitted by Bob.
    let transfer_params = TransferParams::from(vec![concordium_cis2::Transfer {
        from:     ALICE_ADDR,
        to:       Receiver::Account(BOB),
...
fn test_wrap_unwrap() {
    ...
    let update_unwrap = chain
        .contract_update(SIGNER, BOB, BOB_ADDR, Energy::from(10000), UpdateContractPayload {
            amount:       Amount::zero(),
            receive_name: OwnedReceiveName::new_unchecked("gona_token.unwrap".to_string()),
            address:      contract_address,
    ...
    assert_eq!(events, [GonaEvent::Cis2Event(Cis2Event::Burn(BurnEvent {
        token_id: TOKEN_ID_GONA,
        amount:   100.into(),
        owner:    BOB_ADDR,
    })),]);
...
fn test_update_admin() {
    ...
    let update = chain
        .contract_update(SIGNER, ALICE, ALICE_ADDR, Energy::from(10000), UpdateContractPayload {
            amount:       Amount::zero(),
            receive_name: OwnedReceiveName::new_unchecked("gona_token.updateAdmin".to_string()),
            address:      contract_address,
    ...
    // Check that the logs are correct.
    let events = deserialize_update_events(&update);
    assert_eq!(events, [GonaEvent::NewAdmin {
        new_admin: NewAdminEvent {
            new_admin: BOB_ADDR,
        },
    },]);
...
fn test_pause_functionality() {
    ...
    chain
        .contract_update(SIGNER, ALICE, ALICE_ADDR, Energy::from(10000), UpdateContractPayload {
            amount:       Amount::zero(),
            receive_name: OwnedReceiveName::new_unchecked("gona_token.setPaused".to_string()),
            address:      contract_address,
...
fn invoke_view(chain: &mut Chain, contract_address: ContractAddress) -> ReturnBasicState {
    let invoke = chain
        .contract_invoke(ALICE, ALICE_ADDR, Energy::from(10000), UpdateContractPayload {
            amount:       Amount::zero(),
            receive_name: OwnedReceiveName::new_unchecked("gona_token.view".to_string()),
            address:      contract_address,
...
fn get_balances(
    ...
    let invoke = chain
        .contract_invoke(ALICE, ALICE_ADDR, Energy::from(10000), UpdateContractPayload {
            amount:       Amount::zero(),
            receive_name: OwnedReceiveName::new_unchecked("gona_token.balanceOf".to_string()),
            address:      contract_address,
...
fn deserialize_update_events(update: &ContractInvokeSuccess) -> Vec<GonaEvent> {
    update
        .events()
